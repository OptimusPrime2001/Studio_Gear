  ...
  return (
    <main className='flex  flex-col items-center justify-between'>
      ...
      <section className={cn(styles.headingHomeWrapper,'media_width_sm')}>
        ...
        <p className={cn(inter.className,'dark:!text-primary_blur')}><b className='dark:!text-primary_light'>Ben Beckman</b> is a gift & decorations store based in HCMC, Vietnam. Est since 2019. </p>
      </section>
      <section className={cn(styles.mainProduct,'media_width_sm')}>
        {
          listMainProduct.map(item=><div className='product-card' key={item.id}>
            <section className="product-content">
                <h3 className={cn(poppins.className)}>{item.label}</h3>
                <Button reset variant="link" className='shop-now md:mt-3 mt-2'>
                  <span className={inter.className}>Shop now</span>
                  <ArrowRightIcon />
                </Button>
            </section>
